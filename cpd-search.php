...
Copyright 2011-2017 The CPD Team. All rights reserved. Every last one of them.
*/
...
class CPDSearch {
	...
	static function sizeDescription($property) {
		...
		if($sizefrom == $sizeto) {
			return sprintf("%d sq m", $sizefrom);
		}
		else {
			return sprintf("%d to %d sq m", $sizefrom, $sizeto);
		}
	}

	static function sizeDescriptionSqFt($property) {
		$sizefrom = ceil($property->sizefrom * 10.7639);
		$sizeto = ceil($property->sizeto * 10.7639);
		if($sizefrom == $sizeto) {
			return sprintf("%d sq ft", $sizefrom);
		}
		else {
			return sprintf("%d to %d sq ft", $sizefrom, $sizeto);
		}
	}
